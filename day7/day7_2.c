...
#define FILE_SYSTEM_SIZE 70000000
#define SPACE_NEEDED_FOR_UPDATE 30000000
...
void findAndPrintBestToRemove(item* items) {
    item* rootDir = findDirByPath(items, "/");
    int freeSpace = FILE_SYSTEM_SIZE - rootDir->size;
    int spaceNeeded = SPACE_NEEDED_FOR_UPDATE - freeSpace;

    int bestToRemoveSize = INT_MAX;

    for (size_t i=0; i<DIR_ARRAY_LENGTH; i++) {
        if (strcmp(items[i].key, "\0") == 0 && items[i].size == 0) {
            break;
        }
        if (items[i].size >= spaceNeeded && items[i].size < bestToRemoveSize) {
            bestToRemoveSize = items[i].size;
        }
    }

    printf("\n\nfolder size to remove for update 100000: %d\n", bestToRemoveSize);
}
...
int main(int argc, char *argv[]) {
    ...
    findAndPrintBestToRemove(items);
